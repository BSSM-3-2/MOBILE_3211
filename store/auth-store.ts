...
async function getStoredItem(key: string) {
    if (canUseWebStorage) return localStorage.getItem(key);
...
export const useAuthStore = create<AuthState>((set, get) => ({
    ...
    bootstrap: async () => {
        try {
            ...
            const storedRefreshToken = await getStoredItem(REFRESH_KEY);
            set({
                user,
                accessToken: token,
    ...
    signUp: async payload => {
        ...
        try {
            ...
            await setStoredItem(TOKEN_KEY, res.accessToken);
            await setStoredItem(REFRESH_KEY, res.refreshToken);
            set({
                user: res.user,
